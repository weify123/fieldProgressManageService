...
@Component
public class RenYunSyncFieldSupplierProcess implements TaskProcess {
    ...
    public void process(String msgId, String msg) {
        ...
        msgLogEntity.setExchange(sqlServer_field_supplier_mq_exchange);
        msgLogEntity.setRoutingKey(sqlServer_field_supplier_mq_routing_key);
        msgLogEntity.setTryCount(0);
        msgLogEntity.setStatus(INT_FOUR);
        msgLogEntity.setServiceType(INT_SEVEN);
        msgLogEntity.setCreateTime(LocalDateUtil.convertToDate(LocalDateTime.now()));
        rabbitMsgLogMapper.insertSelective(msgLogEntity);

        try {
            supplierAccountService.saveOrUpdate(map);
        } catch (Exception e) {
            logger.error("接收仁云供应商处理失败msg {},e {}", msg, map);
            return;
        }
