...
@Component
public class RenYunSyncFieldAttendanceProcess implements TaskProcess {
    ...
    public void process(String msgId, String msg) {
        ...
        msgLogEntity.setExchange(sqlServer_field_attendance_mq_exchange);
        msgLogEntity.setRoutingKey(sqlServer_field_attendance_mq_routing_key);
        msgLogEntity.setTryCount(0);
        msgLogEntity.setStatus(INT_FOUR);
        msgLogEntity.setServiceType(INT_EIGHT);
        msgLogEntity.setCreateTime(LocalDateUtil.convertToDate(LocalDateTime.now()));
        rabbitMsgLogMapper.insertSelective(msgLogEntity);

        try {
            attendanceService.updateAttendance(map);
        } catch (Exception e) {
            logger.error("接收仁云考勤处理失败msg {},e {}", msg, map);
            return;
        }
